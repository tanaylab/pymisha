...
- **Behavior fix: an overlapping interval scope is now counted once, as R misha counts it.** `gsummary`, `gquantiles`, `gcor`, `gscreen`, `gsample`, `gsegment`, `gwilcox` and `gpartition` took a scope literally, so two intervals covering the same base counted that base twice. `gsummary("dense_track", intervals=<chr1:0-1000, chr1:500-1500>)` reported 40 intervals and sum 2.557778 where R misha reported 30 and 2.337778 - same call, same data, silently different answer. They now canonicalise the scope first, as misha does in all twelve of its aggregating entry points. **If you pass a non-canonical interval set to any of those functions, your numbers change** - they were inflated before. `gextract` is deliberately unchanged: it reports per-interval rows with an `intervalID`, and misha does not unify there either.
- **Database files written from Python now carry the same permissions as those written from C++.** Under the common `umask 022` a track directory came out `0770` while the namespace directory holding it came out `0755`, and `.interv`, `.attributes` and `.db.cache.dirty` came out `0644` - so a colleague could not overwrite from either package what you had saved from Python, and a `0644` `.db.cache.dirty` blocked R's own staleness marker. Set `CONFIG["permissions_umask"] = None` to leave your umask alone.

- **A C++ error raised with no misha call on the stack no longer keeps running.** `verror()` set the Python exception and then returned to the code that raised it, so the caller carried on with invalid state: `_pymisha.pm_interv_register()` before `gdb_init()` segfaulted the interpreter, and a malformed WIG/BedGraph parsed past the bad line and returned a dict with an error already set. It now always raises `pymisha.error`.
- **`gtrack_modify()` no longer edits the track in place.** It was the only pymisha writer that did, so an interrupt, a bad expression or a full disk left the track durably half-old and half-new under its real name, structurally valid, with nothing marking it as damaged. It now writes to a staging copy and commits by rename, as every other writer does. Note the cost: on an indexed database all chromosomes of a track share one file, so the staging copy is the size of the track.
- **A track directory that changes - removed, recreated, renamed, converted - is no longer read through its old layout.** The parsed track index was memoised per directory for the life of the process and nothing ever dropped it, so `gtrack_rm` + recreate, `gtrack_mv` and the indexed converters could leave later reads failing with `Cannot open .../track.dat` or, worse, quietly returning the wrong rows. Restarting the interpreter was the only cure.
- **Track writes now report a full disk instead of silently truncating.** Dense and sparse track files were finalised through `fclose()`, where a deferred `ENOSPC` has nothing left to raise it to; creation reported success and the track read back short.
- **Track creation now flushes to stable storage before publishing the new track.** `rename()` is atomic against other processes but not against a machine dying, which could leave a completed track name pointing at data still in the page cache. Costs under 1% of a track creation, and nothing at all on NFS.
- **A failed call no longer changes the behaviour of later, unrelated ones.** Calling anything that needs a database before `gdb_init()` leaked an internal reference count, after which every subsequent call skipped module initialisation for the life of the process - a different exception type from the identical call, and the umask misha applies to new files silently stopped being set.
