 ...
 * BinFinder.cpp
 *
 * Ported from misha R package
 * Used for binning values in gpartition, gdist, glookup, etc.
 */
...
void BinFinder::init(const double *breaks, unsigned num_breaks, bool include_lowest, bool right)
{
	...
	// keeps m_binsize == Inf. val2bin's uniform-binsize fast path would then
	// compute Inf/Inf = NaN, whose cast to int is undefined behaviour and
