...
class BufferedFile {
public:
	BufferedFile() { init(131072); }  // 128KB buffer — reduced from 2MB to cut per-worker memory for multi-track extraction
...
inline int BufferedFile::getc()
{
	...
	// Cast through unsigned char so a byte >= 0x80 is not sign-extended to a
	// negative int that looks like EOF (-1), which truncated non-ASCII values.
...
{
    // is the new read already cached?
    if (m_virt_pos >= m_sbuf_pos &&
        size <= (uint64_t)std::max<int64_t>(0, m_ebuf_pos - m_virt_pos)) {
...
inline uint64_t BufferedFile::write(const void *ptr, uint64_t size)
{
	...
	if (retv) {
			...
			if (std::max(m_virt_pos, m_sbuf_pos) <
			    std::min(m_virt_pos + (int64_t)retv, m_ebuf_pos))
				m_sbuf_pos = m_ebuf_pos = 0;
...
inline int BufferedFile::seek(int64_t offset, int whence)
{
	...
	case SEEK_END:
		...
		m_virt_pos = m_file_size - offset;
		break;
	case SEEK_CUR:
		if (m_virt_pos + offset < 0 || m_virt_pos + offset > m_file_size) {
			errno = EINVAL;
			return -1;
		}
		m_virt_pos += offset;
