                                                                           ...
                                                                           size_t interval_length)
{
    ...
    // reaching the threshold suffices.) Matches R misha 5.6.10 (19c51158).
    const bool below_bidirect = (m_direction == Direction::BELOW) &&
                                scan_forward && scan_reverse && need_min;
    ...
    for (size_t offset = 0; offset < max_start; ++offset) {
        ...
        if (below_bidirect) {
            ...
            // Prefilter is for the ABOVE pigeonhole bound, so it is skipped here.
            ...
            // The "winning" strand is the one that bounds the answer (the harder one).
            ...
            // PWM_MAX_EDITS uses need_min=false → not relevant in this branch.
        } else {
            if (scan_forward) {
                if (need_min) {
                    bool pass_score_filter = true;
