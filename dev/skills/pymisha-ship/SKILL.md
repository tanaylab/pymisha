...
Ship changes from the `dev` branch to a clean `main` branch via `read-tree`, excluding development-only files.
...
### Post-ship steps (always do these after shipping)
...
git rm -rf dev/
git rm -f CLAUDE.md AGENTS.md 2>/dev/null
